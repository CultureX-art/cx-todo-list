...
export class AIResponseTracker {
  ...
  completeRequest(requestId: string, response: string, tokenUsage: any): void {
  ...
  generateUsageReport(timeRange: { start: Date; end: Date }): UsageReport {
    const relevantResponses = this.responses.filter(
      (r) =>
    ...
    return {
      ...
      totalTokens: relevantResponses.reduce(
  ...
  private calculateAverageResponseTime(responses: AIResponse[]): number {
    ...
    const totalTime = responses.reduce((sum, r) => {
      if (r.responseTimestamp && r.requestTimestamp) {
        return (
          sum + (r.responseTimestamp.getTime() - r.requestTimestamp.getTime())
        );
      }
      return sum;
    }, 0);
  ...
  private generateStageBreakdown(
  ...
  private generateModelBreakdown(
    responses: AIResponse[],
  ): Record<string, number> {
    return responses.reduce(
      (breakdown, r) => {
...
export class TimelineTracker {
  ...
  logEvent(
    event: Omit<TimelineEvent, "id" | "timestamp" | "relatedEvents">,
  ): string {
  ...
  generateVelocityReport(timeRange: {
    start: Date;
    end: Date;
  }): VelocityReport {
  ...
  private calculateAverageStageTime(
    ...
    for (let stage = 1; stage <= 8; stage++) {
      ...
      const startEvents = stageEvents.filter(
        (e) => e.eventType === "stage_start",
      );
      const endEvents = stageEvents.filter(
        (e) => e.eventType === "stage_complete",
      );
      ...
      for (const start of startEvents) {
        ...
        if (correspondingEnd) {
          times.push(
            correspondingEnd.timestamp.getTime() - start.timestamp.getTime(),
          );
        }
      }
  ...
  private calculateActivityBreakdown(events: TimelineEvent[]): {
    ai: number;
    human: number;
    system: number;
  } {
...
export class AuditDashboard {
  ...
  private async getOverviewStats(
    ...
    return {
      ...
      totalAiRequests:
        this.responseTracker.getResponsesByCorrelation("").length,
    };
  }
  ...
  private countActiveFeatures(events: TimelineEvent[]): number {
    ...
    const activeFeatures = new Set(
      recentEvents.map((e) => e.metadata.featureId),
    );
